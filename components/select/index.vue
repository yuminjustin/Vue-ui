<template>
  <div class="_selectbox">
    <div class="btn-group">
      <button class="btn btn-default btn-sm dropdown-toggle" type="button" @click="open" ref="btn">
       {{text}}
       <span class="caret"></span>
    </button>
      <ul is="List" v-for="(data,idx) in lists" :idx="idx" :data="data" :pd="placeholder" :search="searchs[idx]" :selected="selecteds[idx]"
        :muti="ismuti" :show="show&&data.length" @select="select" :style="{ left: idx*160 + 'px' }">
        </ul>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      select(item, idx) {
        ...
        if(!this.son && !this.ismuti) this.show = !1;
        isPush && !this.ismuti && this.pushOut()
      },
      getData() {
        if (!this.selected) {
