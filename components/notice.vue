<script>
  ...
  const Notice = Vue.extend({
	...
	data(){
	   let cn,icon
	   switch(this.type){
	      case "load": cn = "info";icon = "refresh";break;
		  case "error": cn = "danger";icon = "remove-sign";break;
		  case "warn": cn = "warning";icon = "exclamation-sign";break;
		  case "normal": cn = this.type;icon = "info-sign";break;
		  case "success": cn = this.type;icon = "ok-sign";break;
	   }
	   return {
	       cn: 'alert-'+cn,
		   icon:'glyphicon-'+icon
	...
	template:`<div :class="['_notice','alert',cn]" role="alert" @click="clickClose?close():''">
  ...
  export default {
     ...
     methods:{
	   open(cfg){
	      ...
	      $notice = new Notice({propsData:cfg}).$mount()
